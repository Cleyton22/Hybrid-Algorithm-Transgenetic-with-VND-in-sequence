...
void transgeneticgVndHybridMainLoop(Vertice *allVertices, int numVertices, double alpha, double gama){
    ...
    ListSolution *allNonDominatedSolutionsVnd = createListSolution();
    ...
    for (NodeSolution *node = allNonDominatedSolutions->head; node != NULL; node = node->next){
        ...
        while (k <= KMAX){
            int dominance = 0;
            Solution *newSolution = copySolution(node->solution);
            ...
            switch (k){
                case 1:
                    ...
                    onlyAddNonDominatedSolutions(allNonDominatedSolutionsVnd, newSolution);

                    dominance = strictDominance(newSolution, node->solution);

                    if (dominance == 1){
                        node->solution->travelCost = newSolution->travelCost;
                        node->solution->totalLatency = newSolution->totalLatency;
                        node->solution->totalProfit = newSolution->totalProfit;
                        node->solution->depotPosition = newSolution->depotPosition;

                        for (int j = 0; j < node->solution->amountVertices; j++){
                            node->solution->vertices[j] = newSolution->vertices[j];
                        }
    ...
    for (NodeSolution *node = allNonDominatedSolutionsVnd->head; node != NULL; node = node->next){
    ...
    printf("Algoritmo transgenetico com vnd sequencial levou %f segundos para executar \n", cpuTimeUsed);
}
...
int main() {
    ...
    transgeneticgVndHybridMainLoop(allVertices, numVertices, alpha, gama);
